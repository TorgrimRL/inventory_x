...
class InventoryView(views.APIView):
    ...
    def get(self, request: Request) -> Response:
        try:
            ...
            logger.error(f"Failed to fetch inventory items: {str(e)}")
            return Response(
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            ...
            logger.warning(f"Business validation failed: {str(e)}")
            return Response(
                {"detail": str(e)},
                status=status.HTTP_400_BAD_REQUEST,
            )
            ...
            logger.exception(f"Failed to create inventory item: {str(e)}")
            return Response(
                {"detail": "Internal processing error."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
